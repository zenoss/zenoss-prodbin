...
def default_value_converter(value):
    return value


def last_part_of_path_value_converter(value):
...
GLOBAL_CATALOG_TRANSLATIONS = [
    ...
    LegacyFieldTranslation(
        old="allowedRolesAndUsers", new="allowedRolesAndUsers"
    ),
    ...
    # LegacyFieldTranslation(
    #     old="productKeys", new="productKeys"
    # ),
...
IP_SEARCH_CATALOG_TRANSLATIONS = [
    LegacyFieldTranslation(
        old="path", new="uid", value_converter=TranslationValueConverter()
    ),
    LegacyFieldTranslation(
        old="ipAddressAsInt",
        new="decimal_ipAddress",
        ...
        if value and old in self.old_fields:
            ...
            if converter != default_value_converter:
                ...
                if isinstance(value, six.string_types):
                    ...
                    # Not likely since only uid uses a search converter
                    # (for now....)
                    pass
                return new_value
        return value
...
class LegacyCatalogAdapter(SimpleItem):
    ...
    def __init__(self, context, zcatalog_name=None):
        ...
        @param zcatalog_name: string representing the legacyCatalog
            we are adapting.
        """
    ...
    def _adapt_advancedQuery_query(self, translator, query):
        # we need to translate the legacy catalog fields to
        ...
        if isinstance(query, And) or isinstance(query, Or):
            ...
            query._term = translator.convert_search_value(
                old_field_name, query._term
            )
        return query
    ...
    def search(self, query, sort_index=None, reverse=False, limit=None, **kw):
        """
        Perform search on Model Catalog tweaking query and results to match
        legacy indexes and expected data format
        """
        model_catalog = self._get_model_catalog()
        ...
        dict_query = {
            name: value for name, value in kw.items() if name in old_fields
        }
        ...
        if dict_query:
            if not query:
                ...
                aq_from_dict = And(
                    *[Eq(name, value) for name, value in dict_query.items()]
                )
