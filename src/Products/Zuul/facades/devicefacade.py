...
class DeviceFacade(TreeFacade):
    ...
    def addLocationOrganizer(self, contextUid, id, description="", address=""):
        properties = {"address": address} if address else None
        return super(DeviceFacade, self).addOrganizer(
            contextUid, id, description, properties=properties
        )
    ...
    def getDevTypes(self, uid):
        ...
        for org in organizers:
            ...
            if not hasattr(aq_base(org), "devtypes") or not org.devtypes:
                devtypes.append(
                    {
                        "value": getUtility(IVirtualRoot).ensure_virtual_root(
                            org_id
                        ),
