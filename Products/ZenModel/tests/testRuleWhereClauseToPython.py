    ...
    def setUp(self):
        ...
        self.test_rules = [
            ...
            ("(prodState < 1000) and (devicePriority < 5) and (facility = 11) and (eventClassKey != '/Status/Perf') and (eventKey like '%/Status%') and (agent = 'zenhub') and (manager like '%manager_ends_with') and (deviceClass like '/%') and (message not like '%msg doesnt contain%') and (eventState < 1) and (ntevid like '%ntevid_test%') and (severity < 5) and (eventClass like '/Status/Heartbeat%') and (summary like '%foobaz%') and (priority = 0) and (deviceGroups like '%|/%') and (location like '%loc_contains%') and (ownerId = 'admin' or ownerId = 'tester') and (ipAddress like '%192.168%') and (systems like '%|/%')",
             "(dev.production_state < 1000) and (dev.priority < 5) and (evt.syslog_facility == 11) and (evt.event_class_key != '/Status/Perf') and ('/Status' in evt.event_key) and (evt.agent == 'zenhub') and (evt.monitor.endswith('manager_ends_with')) and (deviceClass.beginswith('/')) and ('msg doesnt contain' not in evt.message) and (evt.status < 1) and ('ntevid_test' in evt.nt_event_code) and (evt.severity < 5) and (evt.event_class.beginswith('/Status/Heartbeat')) and ('foobaz' in evt.summary) and (evt.syslog_priority == 0) and ('/' in deviceGroups) and ('loc_contains' in location) and (evt.current_user_name == 'admin') or (evt.current_user_name == 'tester') and ('192.168' in ipAddress) and ('/' in systems)"),
            ...
            ("(prodState = 1000) and (deviceClass like '/Network%' or deviceClass like '/Server%') and (eventState = 0) and (severity >= 5) and (summary not like '%System Fault: FAN FAULT is detected.%') and (deviceGroups like '%|/test%' or deviceGroups like '%|/another/test%')",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network')) or (deviceClass.beginswith('/Server')) and (evt.status == 0) and (evt.severity >= 5) and ('System Fault: FAN FAULT is detected.' not in evt.summary) and ('/test' in deviceGroups) or ('/another/test' in deviceGroups)"),

            ("(prodState = 1000) and (deviceClass like '/Network%') and (device != 'telcotesting1.sat6') and (eventState = 0) and (severity >= 5)",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network')) and (dev.name != 'telcotesting1.sat6') and (evt.status == 0) and (evt.severity >= 5)"),
            ...
            ("(prodState = 1000) and (deviceClass like '/Network%') and (device not like '%stl3%') and (eventState = 0) and (severity >= 5) and (summary not like '%System Fault: FAN FAULT is detected.%') and (deviceGroups not like '%|/test%')",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network')) and ('stl3' not in dev.name) and (evt.status == 0) and (evt.severity >= 5) and ('System Fault: FAN FAULT is detected.' not in evt.summary) and ('/test' not in deviceGroups)"),

            ("(prodState = 1000) and (deviceClass like '/Network%') and (eventState = 0) and (severity >= 4) and (deviceGroups not like '%|/test%')",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network')) and (evt.status == 0) and (evt.severity >= 4) and ('/test' not in deviceGroups)"),

            ("(prodState = 1000) and (deviceClass like '/Network%') and (eventState = 0) and (severity >= 5) and (deviceGroups not like '%|/another/test%')",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network')) and (evt.status == 0) and (evt.severity >= 5) and ('/another/test' not in deviceGroups)"),

            ("(prodState = 1000) and (deviceClass like '/Network%') and (eventState = 0) and (severity >= 5) and (deviceGroups not like '%|/test%')",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network')) and (evt.status == 0) and (evt.severity >= 5) and ('/test' not in deviceGroups)"),
            ...
            ("(prodState = 1000) and (deviceClass like '/Network%') and (eventState = 0) and (severity >= 5) and (summary not like '%System Fault: FAN FAULT is detected.%') and (deviceGroups not like '%|/test%')",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network')) and (evt.status == 0) and (evt.severity >= 5) and ('System Fault: FAN FAULT is detected.' not in evt.summary) and ('/test' not in deviceGroups)"),
            ...
            ("(prodState = 1000) and (deviceClass like '/Network%') and (eventState = 0) and (severity >= 5) and (summary not like '%System Fault: FAN FAULT is detected.%')",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network')) and (evt.status == 0) and (evt.severity >= 5) and ('System Fault: FAN FAULT is detected.' not in evt.summary)"),
            ...
            ("(prodState = 1000) and (deviceClass like '/Network%') and (eventState = 0) and (severity >= 5) and (summary not like '%threshold of Over%' or summary not like '%fan fault%')",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network')) and (evt.status == 0) and (evt.severity >= 5) and ('threshold of Over' not in evt.summary) or ('fan fault' not in evt.summary)"),

            ("(prodState = 1000) and (deviceClass like '/Network%') and (eventState = 0) and (severity >= 5)",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network')) and (evt.status == 0) and (evt.severity >= 5)"),

            ("(prodState = 1000) and (deviceClass like '/Network/Router%') and (eventState = 0) and (severity >= 5)",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Network/Router')) and (evt.status == 0) and (evt.severity >= 5)"),

            ("(prodState = 1000) and (deviceClass like '/Server%') and (device != 'localhost') and (eventState = 0) and (count > 1) and (severity >= 4)",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Server')) and (dev.name != 'localhost') and (evt.status == 0) and (evt.count > 1) and (evt.severity >= 4)"),

            ("(prodState = 1000) and (deviceClass like '/Server%') and (eventState = 0) and (count > 1) and (severity >= 4)",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Server')) and (evt.status == 0) and (evt.count > 1) and (evt.severity >= 4)"),

            ("(prodState = 1000) and (deviceClass like '/Server%') and (eventState = 0) and (severity >= 4)",
             "(dev.production_state == 1000) and (deviceClass.beginswith('/Server')) and (evt.status == 0) and (evt.severity >= 4)"),
            ...
            ("(prodState = 1000) and (eventClass like '/Status/Perf%') and (eventState = 0) and (summary like '%threshold of Over%') and (deviceGroups like '%|/test%')",
             "(dev.production_state == 1000) and (evt.event_class.beginswith('/Status/Perf')) and (evt.status == 0) and ('threshold of Over' in evt.summary) and ('/test' in deviceGroups)"),
            ...
            ("(prodState = 1000) and (eventState = 0) and (eventClass like '/Status/Perf%') and (summary like '%threshold of Over%') and (deviceGroups like '%|/%') and (systems like '%|/foo%' or systems like '%|/foo/bar%')",
             "(dev.production_state == 1000) and (evt.status == 0) and (evt.event_class.beginswith('/Status/Perf')) and ('threshold of Over' in evt.summary) and ('/' in deviceGroups) and ('/foo' in systems) or ('/foo/bar' in systems)"),
