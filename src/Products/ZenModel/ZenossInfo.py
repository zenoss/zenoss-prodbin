...
class ZenossInfo(ZenModelItem, SimpleItem):
    ...
    security.declarePublic("getZenossImageBuild")
    ...
    def getControlCenterVersion(self):
        ...
        if os.path.exists("/serviced/serviced"):
            try:
                # Note the port given below is not used when getting the
                # version, but overrides the SERVICED_UI_PORT env variable
                # that can cause it to fail cli parameter validation.
                ...
                for line in output.split("\n"):
                    ...
                    if splitLine[0] == "Version:":
                        version = Version.parse(
                            "ControlCenter {}".format(splitLine[1])
                        )
                        break
        ...
        try:
            ...
            if data:
                ...
                if match:
                    ...
                    return Version(
                        name, int(major), int(minor), int(micro), 0, versionstr
                    )
    ...
    daemon_tooltips = {
        "zeoctl": (
            "Zope Enterprise Objects server (shares database between Zope "
            "instances)"
        ),
        ...
        "zenactiond": (
            "Receives signals from processed events to execute notifications."
        ),
        "zentrap": "Receives SNMP traps and turns them into events",
        "zenmodeler": "Configuration collection and configuration",
        "zenperfsnmp": (
            "High performance asynchronous SNMP performance collection"
        ),
        "zencommand": (
            "Runs plug-ins on the local box or on remote boxes " "through SSH"
        ),
        "zenprocess": "Process monitoring using SNMP host resources MIB",
        "zendisc": (
            "Discover the network topology to find active IPs and devices"
        ),
        ...
        "zenpop3": (
            "Connect via pop3 to an e-mail server and convert messages "
            "to Zenoss events"
        ),
    }
    ...
    def _getDaemonList(self):
        ...
        daemons = []
        for line in os.popen("%s list" % masterScript).readlines():  # noqa: S605
    ...
    def getZenossDaemonConfigs(self):
        ...
        """
        return [{"name": x} for x in self._getDaemonList()]
    ...
    def getLogData(self, daemon, kb=500):
        ...
        if (
            daemon != "event"
            and self.getDaemonName(daemon) not in self._getDaemonList()
        ):
            return ""
    ...
    def show_daemon_xml_configs(self, daemon, REQUEST=None):
        ...
        try:
            # Always recreate the defaults file in order to avoid
            # caching issues.
            log.debug("Creating XML config file for %s", daemon)
            make_xml = " ".join(
                [binPath(daemon), "genxmlconfigs", ">", xml_default_name]
            )
            proc = Popen(make_xml, shell=True, stdout=PIPE, stderr=PIPE)  # noqa: S602
        ...
        try:
            ...
            info = traceback.format_exc()
            msg = (
                "Unable to obtain current configuration from %s because %s"
                % (configfile, info)
            )
            log.error(msg)
            messaging.IMessageSender(self).sendToBrowser(
        ...
        try:
            ...
            info = traceback.format_exc()
            msg = (
                "Unable to merge XML defaults with config file"
    ...
    def save_daemon_configs(self, REQUEST=None, **kwargs):
        ...
        if not self.isValidDaemon(daemon):
            messaging.IMessageSender(self).sendToBrowser(
                "Internal Error",
                "%r is not a valid daemon name" % daemon,
    ...
    def manage_daemonAction(self, REQUEST):
        ...
        if not self.isValidDaemon(daemonName):
            messaging.IMessageSender(self).sendToBrowser(
                "Internal Error",
                "%r is not a valid daemon name" % daemonName,
                priority=messaging.CRITICAL,
            )
            return self.callZenScreen(REQUEST)
    ...
    def manage_checkVersion(
        self, optInOut=False, optInOutMetrics=False, REQUEST=None
    ):
    ...
    def versionBehind(self):
        if not self.dmd.availableVersion:
            return False
        if (
            Version.parse("Zenoss " + self.dmd.availableVersion)
            > self.getZenossVersion()
        ):
            return True
        return False
