    ...
    def cutover(self, dmd):
        ...
        for svc in svcs:
            ...
            if not filter(lambda x: x.pointID == "eventParserDroppedCount", gc.datapoints):
                log.info(
                    "%s service: Adding 'Parser Dropped Events' graphpoint to the"
                ...
                log.info(
                    "%s service: 'Parser Dropped Events' graphpoint exists"
            ...
            if mc is None:
                ...
                if not filter(lambda x: x.ID == "eventParserDroppedCount", mc.metrics):
                    log.info(
                        "%s service: Adding 'Parser Dropped Events' "
                        "metric", svc.name)
                    mc.metrics.append(
                        Metric(
                    ...
                    log.info(
                        "%s service: 'Parser Dropped Events' metric "
