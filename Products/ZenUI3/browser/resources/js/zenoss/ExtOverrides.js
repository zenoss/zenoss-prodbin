    ...
    */
    ...
    Ext.data.Field.override({
        constructor: function () {
            ...
            if (me.type === Ext.data.Types.AUTO) {
                var customConvert = me.convert;

                me.convert = function (value, record) {
                    if (Ext.isObject(value) && 'uid' in value) {
                        value['uid'] = Zenoss.render.link(null, value['uid']);
                    }

                    if (me.hasCustomConvert) {
                        return customConvert.apply(me, arguments);
                    }
    ...
    Ext.data.reader.Reader.override({
        addVirtualRoot: function (records) {
            Ext.Array.each(records, function(record, index) {
                var uid = record.get('uid');

                if (uid) {
                    record.set({
                        uid: Zenoss.render.link(null, uid)
                    });
                }
            });
        },
        ...
        extractData: function (root) {
            var me = this,
                records = me.callParent(arguments);

            me.addVirtualRoot(records);
