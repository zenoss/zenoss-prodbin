...
from Products.ZenUtils.PASUtils import (
    activatePluginForInterfaces,
    movePluginToTop,
)
...
rbac_pattern = re.compile(r"^(internal:)?(CZ[0-9]+):(.+)")
...
class Auth0(BasePlugin):
    ...
    def getRoleAssignments(roles=None):
        """
        This looks for RBAC style roles in the role list ("CZ0:CZAdmin"), and
        uses those if they exist.  If they don't exist, return the roles as-is
        (they may contain older style roles, ie, "CZAdmin" for Zenoss-com
        connections).

        :param roles: an array of roles, possibly in RBAC format (strings),
            [ "CZ0:ZenManager", "CZ0:ZenUser", .. ]
        ...
        # RBAC style roles are in the form: "CZ#:ZenManager". If our user has
        # any RBAC style role mappings, we need to return only RBAC assigned
        # roles.  This covers the case where we were assigned CZ1:ZenManager
        # and not assigned any roles for CZ0.  We shouldn't return the older
        # gsuite group mappings for this user in this case.
        ...
        # If there are any RBAC roles assigned,
        # use only the RBAC roles for this CZ.
        if matches:
            # for an RBAC role: "CZ0:ZenManager",
            # match.group(3) = "ZenManager", match.group(2) = "CZ0"
            # if this has the optional beginning "internal:CZ0:ZenManager",
            # match(1) is "internal:" and unused.
            return [
                match.group(3)
                for match in matches
                if match.group(2) == cz_prefix
            ]
    ...
    def storeToken(token, request, conf):
        ...
        try:
            ...
            tenant = payload.get(
                tenantkey, None
            )  # ie: "https://dev.zing.ninja/tenant": "alphacorp", in jwt
            # Check the jwt for the globalkey setting.
            is_global_key = payload.get(
                "https://dev.zing.ninja/globalkey", None
            )
            if not is_global_key and not tenant:
                log.warn(
                    "No auth0 tenant specified in jwt for tenantkey: %s",
                    tenantkey,
                )
                Auth0.removeToken(request)
                return None
            # + Get the whitelist from global.conf and verify that
            # it's in the list.
            whitelist = conf.get("whitelist", [])
            if not is_global_key and tenant not in whitelist:
                log.warn(
                    "Tenant %s is invalid. Not in whitelist: %s",
                    tenant,
                    whitelist,
                )
                Auth0.removeToken(request)
            ...
            # use the email as the userid, if defined,
            # otherwise fall back to parsing the sub field.
            if email:
                sessionInfo.userid = email
            else:
                sessionInfo.userid = (
                    payload["sub"].encode("utf8").split("|")[-1]
                )
            sessionInfo.expiration = payload["exp"]
            sessionInfo.roles = Auth0.getRoleAssignments(
                payload.get("https://zenoss.com/roles", [])
            )
            return sessionInfo
    ...
    def resetCredentials(self, request, response):
        ...
        NOTE:
        Logging out of the UI calls
        Products/ZenModel/skins/zenmodel/logoutUser.py, which calls
        resetCredentials, this bypasses the PAS logout.
        """
    ...
    def extractCredentials(self, request):
        ...
        # ZING-821: We need to verify that the session data we've cached
        # matches the token expiration.  If the expiration cookie is set,
        # make sure it matches our cache - otherwise we need to parse the
        # access token again.
        token_expiration = request.cookies.get(Auth0.zc_token_exp_key, None)
        if sessionInfo and token_expiration:
            # ZING stores the expiration *1000 instead of the expiration
            # value (why??), so we have to adjust to compare to our cached
            # session expiration from the token.
            token_expiration = int(token_expiration) / 1000
            if not sessionInfo.expiration == token_expiration:
                log.info(
                    "Token expiration does not match stored expiration. "
                    "Parsing token again."
                )
                sessionInfo = Auth0.storeToken(token, request, conf)
        ...
        if not sessionInfo or not sessionInfo.userid:
            log.debug(
                "No userid found in sessionInfo - not directing to Auth0 login"
            )
            Auth0.removeToken(request)
            return {}

        if time.time() > sessionInfo.expiration:
            # The stored session data is invalid, and we're using Auth0;
            # remove the Auth0 data from the session.
            Auth0.removeToken(request)
            return {}
    ...
    def challenge(self, request, response):
        ...
        if sessionInfo and len(sessionInfo.roles) < 1:
            ...
            # See https://github.com/zenoss/zing-web/blob/feature/delegateToCZModal/packages/z-common/src/mixins/ZErrorCodes.js#L6  # noqa E501
        ...
        # access to this specific resource. Return 401, which would be handled by Zope.
    ...
    def getRolesForPrincipal(self, principal, request=None):
        ...
        if not sessionInfo.roles:
            ...
            return ()
        # to avoid the issue in ZEN-31732
        # - CZ-only users can still see all the events
    ...
    def getPropertiesForUser(self, user, request=None):
        ...
        sessionInfo = request.SESSION.get(Auth0.session_key)
        if not sessionInfo:
            log.debug("No Auth0 session - not getting properties for user")
            return {}
        userid = getattr(sessionInfo, "userid", None)
        if not userid:
            log.debug("No 'userid' in Auth0 session")
            return {}
