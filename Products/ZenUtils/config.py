...
def add_config_arguments(parser, basename=None):
    ...
    parser.add_argument(
        "-C",
        "--configfile",
        default=os.path.join(zenPath("etc"), "{}.conf".format(basename)),
...
def getConfigFromArguments(parser, args):
    ...
    dest_names = {
        long_name: dest_name for long_name, dest_name, _, _ in options
    }
    xforms = {long_name: xform for long_name, _, xform, _ in options}
    defaults = {long_name: default for long_name, _, _, default in options}
