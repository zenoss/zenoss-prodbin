...
class IZingConnectorClient(Interface):
    ...
    def send_facts(self, facts, ping):
        """Sends a list of facts to zing-connector.

        Returns boolean indicating if the request succeeded.
        """

    def send_facts_in_batches(self, facts, batch_size):
        """Sends a list of facts to zing-connector in batches.

        Returns boolean indicating if the requests succeeded.
        """

    def send_fact_generator_in_batches(
        self, fact_gen, batch_size, external_log=None
    ):
        """Sends facts given by the passed generator to zing-connector.
