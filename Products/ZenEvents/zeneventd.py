...
def monkey_patch_rotatingfilehandler():
    try:
        ...
        warn("ConcurrentLogHandler package not installed. Using"
...
from Products.ZenCollector.utils.maintenance import (
    MaintenanceCycle, maintenanceBuildOptions, QueueHeartbeatSender
)
...
from zenoss.protocols.protobufs.zep_pb2 import (
    ZepRawEvent, Event, STATUS_DROPPED
)
...
from Products.ZenEvents.events2.processing import (
    Manager, EventPluginPipe, CheckInputPipe, IdentifierPipe,
    AddDeviceContextAndTagsPipe, TransformAndReidentPipe, TransformPipe,
    UpdateDeviceContextAndTagsPipe, AssignDefaultEventClassAndTagPipe,
    FingerprintPipe, SerializeContextPipe, ClearClassRefreshPipe,
...
from Products.ZenEvents.daemonlifecycle import (
    DaemonCreatedEvent, SigTermEvent, SigUsr1Event,
    DaemonStartRunEvent, BuildOptionsEvent
)
        ...
        try:
            ...
            while not processed:
                try:
                    ...
                    # _manager throws Attribute errors if connection to
                    # zope is lost - reset and retry ONE time
                    if retry:
                        retry = False
            ...
            failReportEvent = {
                ...
                'fingerprint': '|'.join(
                    ['zeneventd', 'processMessage', repr(e)]
                ),
                # Don't send the *same* event class or we crash endlessly
                'eventClass': '/',
