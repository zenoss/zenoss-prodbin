...
__doc__ = """EnableZproxyNginxPagespeed
Turns on `pagespeed` setting in Zproxy.
"""
...
class EnableZproxyNginxPagespeed(Migrate.Step):
    ...
    def cutover(self, dmd):
        ...
        try:
            config = filter(lambda conf: conf.name == config_name, zproxy.configFiles)[0]
        except IndexError:
