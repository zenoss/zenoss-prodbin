...
class FixZenactiondLogFilter(Migrate.Step):
    """Correct the LogFilter for /opt/zenoss/log/zenactiond.log"""
    ...
    def cutover(self, dmd):
        ...
        services = filter(lambda s: s.name == "zenactiond", ctx.services)
        log.info("Found %d services named 'zenactiond'." % len(services))

        changed = False
        for service in services:
            for logConfig in service.logConfigs:
                if logConfig.path == "/opt/zenoss/log/zenactiond.log":
