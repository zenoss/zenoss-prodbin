...
class AddSnmpLastCollectionIndex(Migrate.Step):
    ...
    def cutover(self, dmd):
        ...
        search_results = IModelCatalogTool(dmd).devices.search(limit=1, fields=["snmpLastCollection"])
        if search_results.total > 0 and search_results.results.next().snmpLastCollection is None:
            log.info("Adding index for last model time, this can take a while.")
