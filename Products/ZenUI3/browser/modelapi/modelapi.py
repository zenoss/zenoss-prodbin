...
class RabbitQueues(BrowserView):
    ...
    def __call__(self):
        ...
        queueData = json.loads(requests.get('http://localhost:15672/api/queues/%2Fzenoss',
                                            auth=requests.auth.HTTPBasicAuth(user, password)).content)
        queueMaps = [dict(id=queue['name']) for queue in queueData if queue['durable']]
...
class ZenossRMDevice(BrowserView):
    ...
    def __call__(self):
        modelInfo = {}
        modelInfo['controlplaneTenantId'] = os.environ.get('CONTROLPLANE_TENANT_ID', None)
...
class ZenModeler(BaseApiView):
    ...
    def _services(self):
        return (
            ('zenModelers', 'zenmodeler'),
        )

    def _getServices(self, svcName):
        idFormat = '{}_{}'
        titleFormat = '{} - {}'
        return [dict(id=idFormat.format(svc.name, self._appfacade.get(svc.parentId).name),
                     title=titleFormat.format(svc.name, self._appfacade.get(svc.parentId).name),
                     controlplaneServiceId=svc.id,
                     instanceCount=svc.instances,
                     RAMCommitment=getattr(svc, 'RAMCommitment', None),
                     lastModeledState=str(svc.state).lower())
...
class RegionServer(BaseApiView):
    ...
    def _getServices(self, svcName):
        ...
        count = svc.instances
        titleFormat = '{} - {}'
        return [dict(id=str(i),
                     title=titleFormat.format(svc.name, i))
...
class Zope(BaseApiView):
    ...
    def _getServiceInstances(self, name):
        ...
        if not services:
            return []
        svc = services[0]
        data = [dict(id=idFormat.format(name, i),
                     title=titleFormat.format(name, i))
...
class ZingConnector(BaseApiView):
    """
    This view emits the ZingConnector stats of the Zenoss system
    """
    @property
    def _services(self):
        return (
