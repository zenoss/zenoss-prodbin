...
class IQueuePublisher(Interface):
    ...
    def publish(
        ...
        If twisted is running this will use the twisted amqp library,
        otherwise it will be blocking.

        @type  exchange: string
        @param exchange: destination exchange for the amqp server
        @type  routing_key: string
        @param routing_key: Key by which consumers will setup
            the queues to route.
        @type  message: string or Protobuf
        @param message: message we are sending in the queue
        @type  createQueues: list
        @param createQueues: The name of the queues defined in the
            queue schema to create prior to publishing the message.
        @type  mandatory: Boolean.
        @param mandatory: If true, will raise NoRouteException if there
            is no destination queue for the published event.
        @type  headers: dict
...
class IQueueConsumerTask(Interface):
    """A Task that is called once for every message received from the Queue.
    ...
    def processMessage(message):
        ...
        Implementations can call "acknowledge" on the Queue Consumer
