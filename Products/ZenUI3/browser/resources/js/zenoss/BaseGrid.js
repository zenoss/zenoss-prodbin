                ...
                if (values[paramKey]) {
                    ...
                    if (timeParam.indexOf(" TO ") != -1) {
                        ...
                        values[paramKey] = dateValues.map(function(rangeValue){
                            return moment(rangeValue, Zenoss.USER_DATE_FORMAT+' '+Zenoss.USER_TIME_FORMAT).utc().format("YYYY-MM-DD HH:mm:ss")
                        }).join(" TO ");
                    }
                    else {
                        values[paramKey] = moment(timeParam, Zenoss.USER_DATE_FORMAT+' '+Zenoss.USER_TIME_FORMAT).utc().format("YYYY-MM-DD HH:mm:ss")
                    }
                }
            });
            if (!this.grid.store.proxy.extraParams) {
