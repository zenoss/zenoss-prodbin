...
from .ZenossSecurity import ZEN_VIEW, ZEN_COMMON, ZEN_MANAGE_DMD
...
class ZenModelBase(object):
    ...
    @unpublished
    def urlLink(self, text=None, url=None, attrs=None):
        """
    ...
    def getPrimaryDmdId(self, rootName="dmd", subrel=""):
        ...
        if subrel:
            path = filter(lambda x: x != subrel, path)
        return "/" + "/".join(path)
    ...
    def visibleCustPropertyMap(self):
        ...
        return [
            p
            for p in self.zenPropertyMap(pfilt=iscustprop)
